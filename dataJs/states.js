...
function cdp_load(page) {
    ...
    $("#loader").fadeIn('slow');
    $.ajax({
        url: './ajax/locations/states/states_list_ajax.php',
        data: parametros,
    ...
    new Messi('<p class="messi-warning"><i class="icon-warning-sign icon-3x pull-left"></i>' + message_delete_confirm + '<br /><strong>' + message_delete_confirm2 + '</strong></p>', {
        ...
        callback: function (val) {
            if (val === 'Y') {
                $.ajax({
                    type: 'post',
                    url: './ajax/locations/states/states_delete_ajax.php',
                    data: {
                        'id': id,
                    },
                    beforeSend: function () {
...
$("#save_data").on('submit', function (event) {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/locations/states/states_add_ajax.php",
        data: parametros,
        beforeSend: function (objeto) {
        },
...
$("#update_data").on('submit', function (event) {
    ...
    $.ajax({
        type: "POST",
        url: "ajax/locations/states/states_edit_ajax.php",
        data: parametros,
        beforeSend: function (objeto) {
